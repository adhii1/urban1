...
/** "Which ride" — from the first passenger's subscription type. */
function rideTypeLabel(trip: { passengers?: Array<{ subscriptionId?: { subscriptionType?: string } | null }> }) {
  const type = (trip.passengers || []).map((p) => p.subscriptionId?.subscriptionType).find(Boolean);
  return type ? RIDE_TYPE_LABELS[type] || type : null;
}

/** "Which area" — the named service area, never raw coordinates. */
function displayArea(trip: { areaId?: { name?: string } | null }) {
  return trip.areaId?.name || 'Area pending';
}
...
export default function DriverDashboardPage() {
  ...
  return <div>
    ...
    <section className="driver-dashboard-grid">
      ...
      <aside className="driver-glass-card driver-timeline"><h2><CalendarDays size={16} color="#16C15D" style={{ verticalAlign: -3, marginRight: 8 }} /> My Scheduled Shifts</h2>{isError ? <p style={{ color: '#EF4444', fontSize: 13 }}>Could not load your assigned trips.</p> : assigned.length ? assigned.slice(0, 6).map((trip) => <div className="driver-timeline-item" key={trip._id}><time>{trip.pickupTime || displayTime(trip.scheduledTime)}</time><span className="driver-timeline-node" /><div className="driver-timeline-content"><h3>{rideTypeLabel(trip) || displayRoute(trip)}</h3><p><MapPin size={11} style={{ verticalAlign: -1, marginRight: 3 }} /> {displayArea(trip)}</p><span style={{ display: 'inline-block', marginTop: 6, padding: '2px 6px', borderRadius: 6, background: 'rgba(59,130,246,.12)', color: '#2563EB', fontSize: 9, fontWeight: 700 }}>{trip.status}</span></div></div>) : <p style={{ padding: '14px 0', color: 'var(--driver-muted)', fontSize: 13 }}>No assigned or pending shifts.</p>}</aside>
    </section>
  </div>;
}
